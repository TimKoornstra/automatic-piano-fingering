...
def train(data):
    ...
    for episode in range(episodes):
        ...
        for i in range(len(notes)):
            ...
            if (i == 0):
                action_index = env.get_next_action(None, notes[i], notes[i+1], epsilon)
            elif (i == len(notes) - 1):
                action_index = env.get_next_action(notes[i-1], notes[i], None, epsilon)
            else:
                action_index = env.get_next_action(notes[i-1], notes[i], notes[i+1], epsilon)

            # Calculate reward and temporal difference and transition to next state
            reward = env.calculate_reward(notes[i] / 2 + lowest_note, previous_finger, action_index)
            ...
            if (i != 0 and i != len(notes) - 1):
                ...
                temporal_difference = reward + discount_factor * np.max(q_values[notes[i-1], notes[i], notes[i+1]]) - old_q
